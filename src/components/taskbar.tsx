    ...
    if ("getBattery" in navigator) {
      (navigator as any).getBattery().then((battery: any) => {
        const updateBattery = () => {
          ...
          setTimeRemaining(
            battery.charging
              ? `${Math.floor(battery.chargingTime / 60)}:${String(
                  battery.chargingTime % 60
                ).padStart(2, "0")} to full`
              : `${Math.floor(battery.dischargingTime / 60)}:${String(
      ...
      {isOpen && (
        <div className="absolute right-0 top-12 w-72 bg-[#2e2e2e] border border-gray-600 rounded-lg shadow-xl overflow-hidden z-50">
          <div className="p-4 border-b border-gray-600">
            <p className="font-medium">Bobbys Ubuntu</p>
            {batteryLevel !== null ? (
              <div className="flex items-center mt-1 text-sm text-gray-400">
                <FiBattery className="mr-2" />
                <span>
                  {batteryLevel}% ({timeRemaining})
                </span>
              </div>
            ) : (
